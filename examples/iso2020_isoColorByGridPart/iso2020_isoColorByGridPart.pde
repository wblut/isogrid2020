...
void setup() {
  ...
  iso.openKAll();
  colorMode(HSB);
  //Color determined by grid part id
  isoColor=new WB_IsoColor(new HueByPart(),255,255);
}
...
class HueByPart implements WB_ColorChannel{
  float value(WB_IsoGridCell cell, int triangle){
   return (cell.getPart(triangle)*17)%256;
  }  
}
